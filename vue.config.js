...
module.exports = {
  ...
  devServer: {
    ...
    // 代理配置  本地的前端  =》 本地的后端  =》 代理我们向另一个服务器发请求 （行得通）
    proxy: {
  ...
  chainWebpack(config) {
    ...
    // 修改异步代码时，app的js文件名字的hash值会发生变化，这时就不能使用缓存，会减少加载速度，可以下载script-ext-html-webpack-plugin这个插件，加上这些配置。会生成对应的
    // 打包时生成一个体积很小runtime.xxx.js文件，用作映射其他chunk文件，目的是更新后，以较小的代价利用缓存，提升页面加载速度。 可以利用script-ext-html-webpack-plugin将runTime代码生成到行内的html中，不用发送请求
    config
